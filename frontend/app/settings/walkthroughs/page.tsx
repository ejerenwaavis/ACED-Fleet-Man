...
export default function DynamicWalkthrough() {
  ...
  // Find the field marked (or inferred) as the odometer/mileage reading so its value
  // gets sent as top-level `mileage` and actually reaches the vehicle record.
  const getMileageValue = () => {
    if (!template?.items) return undefined;
    const mileageItem = template.items.find((i: any) => i.isMileageField) ||
      template.items.find((i: any) => i.type === 'number' && /mileage|odometer/i.test(i.label || ''));
    return mileageItem ? data[mileageItem.id] : undefined;
  };
